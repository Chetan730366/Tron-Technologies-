...
export default function ProductDetail() {
  ...
  const gallery = [
    product.image,
    product.image.replace("w=600&h=600", "w=600&h=600&sat=-30"),
    product.image.replace("w=600&h=600", "w=600&h=600&blur=10"),
    product.image.replace("w=600&h=600", "w=600&h=600&fit=crop&crop=edges"),
  ];
    ...
    <main className="bg-slate-50 min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-4 py-8 md:py-12">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div className="mt-4 grid grid-cols-4 gap-3">
              {gallery.map((g, i) => (
                <button
                  ...
                  className={`aspect-square rounded-xl overflow-hidden border-2 transition ${i === activeImg ? "border-pink-600 ring-2 ring-pink-200" : "border-slate-200 hover:border-pink-300"
                    }`}
                >
                  <img
                    src={g}
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            <div className="text-xs uppercase tracking-[0.2em] font-bold text-pink-600">{category.name}</div>
            <h1 className="mt-2 text-3xl md:text-4xl font-black text-slate-900 leading-tight">{product.name}</h1>

            <div className="mt-3 flex items-center gap-3 text-sm">
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star key={s} size={16} className={s <= Math.round(product.rating) ? "fill-yellow-400 text-yellow-400" : "text-slate-300"} />
                ))}
            ...
            <div className="mt-6 p-5 rounded-2xl bg-white border border-slate-200">
              <div className="flex items-baseline gap-3 flex-wrap">
                <span className="text-4xl md:text-5xl font-black text-pink-600">₹{product.price.toLocaleString()}</span>
                <span className="text-lg text-slate-400 line-through">₹{mrp.toLocaleString()}</span>
              ...
              <div className="mt-1 text-xs text-slate-500">Inclusive of all taxes · Bulk pricing on request</div>
              ...
              <div className="mt-5 flex items-center gap-4 flex-wrap">
                ...
                <div className="flex items-center border border-slate-300 rounded-full overflow-hidden">
                  <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="w-10 h-10 flex items-center justify-center hover:bg-slate-100 transition">
                    <Minus size={14} />
                  </button>
                  <span className="w-12 text-center font-bold text-slate-900">{qty}</span>
                  <button onClick={() => setQty((q) => q + 1)} className="w-10 h-10 flex items-center justify-center hover:bg-slate-100 transition">
                    <Plus size={14} />
                  </button>
                </div>
                <span className="text-sm text-slate-500">
                  Total: <span className="font-bold text-slate-900">₹{(product.price * qty).toLocaleString()}</span>
                </span>
              </div>
              ...
              <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <a href={waLink} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold shadow-lg shadow-emerald-500/30 transition">
                  <MessageCircle size={18} fill="currentColor" /> WhatsApp Us
                </a>
              </div>

              <a href="tel:+919315974174" className="mt-3 flex items-center justify-center gap-2 px-5 py-3 rounded-xl border-2 border-slate-200 text-slate-700 font-bold hover:border-pink-400 hover:text-pink-600 transition">
            ...
            <div className="mt-5 grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <div key={b.t} className="flex items-center gap-2 p-3 rounded-xl bg-white border border-slate-200 text-sm">
          ...
          <div className="flex border-b border-slate-200 overflow-x-auto no-scrollbar">
            {[
              ...
              <button
                ...
                className={`px-6 py-4 text-sm font-bold uppercase tracking-wider whitespace-nowrap transition relative ${activeTab === key ? "text-pink-600" : "text-slate-500 hover:text-slate-900"
                  }`}
              >
                {label}
                {activeTab === key && (
                  <motion.span layoutId="tab-underline" className="absolute left-4 right-4 bottom-0 h-[3px] bg-pink-600 rounded-t" />
                )}
              </button>
            ))}
          </div>
          ...
          <div className="p-6 md:p-8">
            {activeTab === "desc" && (
              <div className="text-slate-600 leading-relaxed space-y-4">
                <p>
                  The <strong>{product.name}</strong> is a flagship industrial-grade {category.name.toLowerCase()} engineered for demanding garment factories, textile mills and OEM applications. Built using imported raw materials and assembled in our ISO-9001 certified Mumbai facility, every unit is rigorously quality-tested before dispatch.
                </p>
                <p>
                  Designed for round-the-clock operation, this model delivers consistent performance with minimal maintenance — exactly what production lines need to keep moving. With a corrosion-resistant coating, precision-machined fittings and easy plug-and-play installation, it's built to last for years.
                </p>
                <p>
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1">
                            {[1, 2, 3, 4, 5].map((s) => (
                              <Star key={s} size={12} className={s <= rev.r ? "fill-yellow-400 text-yellow-400" : "text-slate-300"} />
                            ))}
                          </div>
                        </div>
                      </div>
        ...
        {related.length > 0 && (
          <div className="mt-16">
            <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
              <div>
                <div className="text-xs uppercase tracking-[0.3em] text-pink-600 font-bold">You may also like</div>
                <h2 className="mt-1 text-2xl md:text-3xl font-black text-slate-900">Related Products</h2>
              </div>
              <Link to={`/category/${category.id}`} className="text-sm font-bold text-pink-600 hover:text-pink-700 inline-flex items-center gap-1">
